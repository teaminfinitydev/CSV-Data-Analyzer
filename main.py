...
class CSVAnalyzer:
    ...
    def setup_ui(self):
        ...
        ttk.Button(control_frame, text="Show Statistics",
                   command=self.show_statistics).grid(row=0, column=1,
                                                      padx=(0, 10))

        ttk.Button(control_frame, text="Create Visualization",
                   command=self.create_visualization).grid(row=0, column=2,
                                                           padx=(0, 10))
        ...
        data_frame = ttk.LabelFrame(content_frame, text="Data Preview",
                                    padding="5")
        ...
        tree_scroll = ttk.Scrollbar(data_frame, orient="vertical",
        ...
        self.analysis_text = tk.Text(analysis_frame, wrap=tk.WORD,
                                     font=('Consolas', 10))
        self.analysis_text.grid(row=0, column=0,
                                sticky=(tk.W, tk.E, tk.N, tk.S))
        ...
        self.status_label = ttk.Label(status_frame,
                                      text="Ready - Load a CSV file to begin")
    ...
    def load_file(self):
        ...
        if file_path:
            try:
                ...
                status_text = (f"Loaded: {filename} - {len(self.df)} rows, "
    ...
    def show_statistics(self):
        ...
        stats_text += f"Memory usage: {memory_kb:.2f} KB\n\n"
    ...
    def create_scatter_plots(self, notebook, numeric_cols):
        ...
        if len(numeric_cols) >= 2:
            ...
            ax.scatter(self.df[x_col], self.df[y_col], alpha=0.6, c='coral',
                       edgecolors='black')
            ax.set_xlabel(x_col)
            ax.set_ylabel(y_col)
        ...
        if file_path:
            try:
                with open(file_path, 'w') as f:
                    ...
                    f.write(f"Dataset Shape: {self.df.shape[0]} rows × "
                    ...
                    for col in self.df.columns:
                        f.write(f"{col}: {self.df[col].dtype}, "
                                f"{self.df[col].isnull().sum()} nulls\n")
